...
TAKE_TREASURE_ENDPOINT = "https://lambda-treasure-hunt.herokuapp.com/api/adv/take/"
GET_STATUS_ENDPOINT = "https://lambda-treasure-hunt.herokuapp.com/api/adv/status/"
CHANGE_NAME_ENDPOINT = "https://lambda-treasure-hunt.herokuapp.com/api/adv/change_name/"
SELL_TREASURES_ENDPOINT = "https://lambda-treasure-hunt.herokuapp.com/api/adv/sell/"

headers = { 'Authorization': 'token 72835a1eadb9d32cc63af8173e157b0a1a4ff7df',
    'content-type': 'application/json'}

def move(current_room_id, direction):
    #set up move request then send
    move_data = {"direction": direction, "next_room_id": str(map[str(current_room_id)][direction])}
...
def take_treasure(treasure):
    ...
    treasure_data_json = json.dumps(treasure_data)
    r = requests.post(url=TAKE_TREASURE_ENDPOINT, data=treasure_data_json, headers=headers)
    print(r.status_code)

def get_status():
    r = requests.post(url=GET_STATUS_ENDPOINT, headers=headers)
    return r.json()
...
def get_path_to_ry(current_room_id):
    path = Queue().bfs(str(current_room_id), "467")
    return path

def get_path_to_store(current_room_id):
    path = Queue().bfs(str(current_room_id), "1")
    return path

def purchase_name_from_ry():
    name_data = {"name":"Lieutenant S.M. Humphrey", "confirm": "aye"}
    json_name_data = json.dumps(name_data)
    requests.post(url=CHANGE_NAME_ENDPOINT, data=json_name_data, headers=headers )

def sell_treasures(current_room):
    treasures = get_status()["inventory"]
    for treasure in treasures:
        treasure_data = {"name": treasure, "confirm": "yes"}
        json_treasure_data = json.dumps(treasure_data)
        requests.post(url=SELL_TREASURES_ENDPOINT, data=json_treasure_data, headers=headers)
        wait(current_room["cooldown"])
...
def traversal_algo():
    ...
    while True:
        if go_to_ry == True:
            path = get_path_to_ry(current_room_id)[1:]
            while len(path) > 0:
                print(current_room_id)
                print(path)
                for key, value in map[str(current_room_id)].items():
                    if str(value) == str(path[0]):
                        current_room = move(current_room_id, key)
                        current_room_id = current_room["room_id"]
                        wait(current_room["cooldown"])
                        path = path[1:]
                        break
            go_to_ry = False
            purchase_name_from_ry()
            break
        if go_to_store == True:
            path = get_path_to_store(current_room_id)[1:]
            while len(path) > 0:
                print(current_room_id)
                print(path)
                for key, value in map[str(current_room_id)].items():
                    if str(value) == str(path[0]):
                        ...
                        break
            go_to_store = False
            sell_treasures(current_room)
            break
        print(current_room_id)
        count = 0
        # get valid exits
        valid_exits = get_exits(current_room)
        ...
        for direction in valid_exits:
            ...
            if map[str(current_room_id)][direction] not in traversal_path:
                current_room = move(int(current_room_id),direction)
                current_room_id = current_room["room_id"]
        ...
        if count == 0:
            ...
            current_room = move(current_room_id, direction)
            current_room_id = current_room["room_id"]
            wait(current_room["cooldown"])
